        ...
        try {
            ...
            if (req.data.Action === 'CREATE') {
                ...
                if (existid.length !== 0) {
                    ...
                    const output = await dbconn.callProcedurePromisified(sp, [retailerType[0].RETAILER_TYPE,req.data.Action, distributorId, req.data.retailerDetails, req.data.retailerAddress, req.data.retailerAttachments]);
                    ...
                    if (output.outputScalar.OUT_SUCCESS !== null) {
                        ...
                        //      await lib_email.sendidealEmail("vaishali.c@intellectbizware.com", sCCEmail, 'html', oEmaiContent.subject, oEmaiContent.emailBody)
                ...
                if (duplicatedata.length === 0) {
                    ...
                    const output = await dbconn.callProcedurePromisified(sp, [null ,req.data.Action, distributorId, req.data.retailerDetails, req.data.retailerAddress, req.data.retailerAttachments]);
//      
                    // isEmailNotificationEnabled = await lib_email.isiDealSettingEnabled(connection, "VM_EMAIL_NOTIFICATION");
